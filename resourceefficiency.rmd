...
```{r}
P<-fillrecycling(recycled,P,nproc,nprod,origin,target)
print(P)
...
```{r,echo=FALSE}
flowanaly<-flowanalysis(P,nprod,recycled,goods,resources)
chainflows<-flowanaly[[1]]
origin<-flowanaly[[2]]
target<-flowanaly[[3]]
suminputs<-flowanaly[[4]]
sumgoods<-flowanaly[[5]]
exportflows<-flowanaly[[6]]
sumlosses<-flowanaly[[7]]
sumresources<-flowanaly[[8]]
nue<-flowanaly[[9]]
```
...
```{r}
lambda<-allocationbyflow(P,goods,origin,chainflows,recycl)
...
```{r,echo=FALSE}
for(i in 1:nfinprod){
    print(burdenproducts[cfinprod[i,1],cfinprod[i,2],drop=FALSE])
    cat("\n")
}
```
With the sum of the burden `r sum(burdenproducts)` being the same as the original total burden.
...
```{r,echo=FALSE}
for(i in 1:nfinprod){
    print(resourcesproducts[cfinprod[i,1],cfinprod[i,2],drop=FALSE])
    cat("\n")
}

```
Again, with the sum of the resources `r sum(resourcesproducts)` being the same as the original total resource.
...
```{r,echo=FALSE}
for(i in 1:nfinprod){
    print(nueproducts[cfinprod[i,1],cfinprod[i,2],drop=FALSE])
    cat("\n")
}

```
The LCA-NUE of the final demanded product is therefore `r nueproducts[cfinprod[nfinprod,1],cfinprod[nfinprod,2]]`
...
supplychain<-supplychainsimple()
egsupply<-exampleaimable(nproc,nprod,nenv)
mfa<-materialflow(supplychain,eggsupply)

lambda<-allocationbyvalue1(nproc,nprod)
lcaap<-lcaapproach(P,nproc,nprod,lambda,chainflows,target)
lambda<-lcaap[[1]]
burden<-lcaap[[2]]
lca<-lcaap[[3]]
print(comparison(mfa,lca))
```
...
mfa<-materialflow(S,E)

lambda<-allocationbyflow(P,goods,origin,chainflows,recycled)
lcaap<-lcaapproach(P,nproc,nprod,lambda,chainflows,target)
lambda<-lcaap[[1]]
burden<-lcaap[[2]]
lca<-lcaap[[3]]
...
In this example ther is an eternal N-input of `r sum(resourcesproducts)` kg N per 'cycle'. All of this is added to the first process (Feed production). Feed processing has two goods that leave the process: feed that goes to process 2 (`r P[1,1]` kg) and exported crop/feed of `r P[4,1]` kg. Thus `r round(100*lambdamain[1,1],0)`% of burden (total N losses) is assigned to feed and `r round(100*lambdamain[4,1],0)`%. Thus the `r P[11,1]` kg of losses are distributed accordingly. The second process generates three goods that are leaving the process: livestock product that is processed in process 3, exported livestock product (co-product) and a co-product that is recycled as fertilizer in process 1. The burden of `r P[11,2]` kg is distrubuted according to the flow strength of N. Process 3 generates the final product and recycles a small amount of N as fertilizer.
```{r,echo=FALSE}
