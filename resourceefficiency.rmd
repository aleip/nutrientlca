...
lam2[4,1]<-P[4,1]*1
lam2[5,2]<-P[5,2]*1
# Recycled products half value
lam2[6,1]<-P[6,1]*0.5
lam2[8,2]<-P[8,2]*0.5
lam2[9,3]<-P[9,3]*0.5

#lam2[,1]<-c( 50*2,    0,     0,40*1.2,    0,10*0,  0,    0,   0)
#lam2[,2]<-c(    0, 20*2,     0,     0,5*0.5,   0,  0, 10*0,   0)
#lam2[,3]<-c(    0,    0,12.5*2,     0,    0,   0,  0,    0, 2*0)
lam2r<-t(t(lam2)/colSums(lam2))
lam2t<-t(t(lam2r)*colSums(V*diag(3)))
lamratio<-colSums(lam2)/colSums(V*diag(3))

# Add consumption for chain flows acc to allocation data
